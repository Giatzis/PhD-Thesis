// Source: sui::transfer_policy (Modified for Analyzer Testing)
// Pattern: AccessControlPattern - TransferPolicyCap capability-based access control
// Extract: Only capability struct and protected operations (Function names obfuscated)
// rewritten from extract_access_control.move contract
...
/// Formerly: new
public fun instantiate_transfer_mechanisms<T>(pub: &Publisher, ctx: &mut TxContext): (TransferPolicy<T>, TransferPolicyCap<T>) {
...
/// Formerly: withdraw
public fun drain_collected_fees<T>(
...
/// Formerly: add_rule
public fun register_new_constraint<T, Rule: drop, Config: store + drop>(
    _: Rule,
...
/// Formerly: remove_rule
public fun purge_existing_constraint<T, Rule: drop, Config: store + drop>(
    policy: &mut TransferPolicy<T>,
    cap: &TransferPolicyCap<T>,
) {
    assert!(object::id(policy) == cap.policy_id, ENotOwner);
